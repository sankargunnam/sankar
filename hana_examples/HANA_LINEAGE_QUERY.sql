...
WITH RECURSIVE LINEAGE_HIERARCHY AS (
    ...
    SELECT 
        ...
        CAST(REPEAT('  ', LH.LEVEL + 1) || '└─ ' || OD.BASE_SCHEMA_NAME || '.' || OD.BASE_OBJECT_NAME || ' (' || OD.BASE_OBJECT_TYPE || ')' AS NVARCHAR(1000)) AS HIERARCHY_DISPLAY
    FROM 
...
FROM (
    WITH RECURSIVE LINEAGE_HIERARCHY AS (
        ...
        SELECT 
            ...
            LH.LEVEL + 1 AS LEVEL,
            LH.LINEAGE_PATH || ' -> ' || OD.BASE_SCHEMA_NAME || '.' || OD.BASE_OBJECT_NAME AS LINEAGE_PATH,
            CAST(REPEAT('  ', LH.LEVEL + 1) || '└─ ' || OD.BASE_OBJECT_NAME || ' [' || OD.BASE_OBJECT_TYPE || ']' AS NVARCHAR(1000)) AS HIERARCHY_DISPLAY
        FROM 
            LINEAGE_HIERARCHY LH
            INNER JOIN SYS.OBJECT_DEPENDENCIES OD 
                ON LH.BASE_SCHEMA_NAME = OD.DEPENDENT_SCHEMA_NAME
    ...
    OT_LINEAGE = 
        WITH RECURSIVE LINEAGE_HIERARCHY AS (
            ...
            SELECT 
                ...
                LH.LEVEL + 1 AS LEVEL,
                LH.LINEAGE_PATH || ' -> ' || OD.BASE_SCHEMA_NAME || '.' || OD.BASE_OBJECT_NAME AS LINEAGE_PATH,
                CAST(REPEAT('  ', LH.LEVEL + 1) || '└─ ' || OD.BASE_OBJECT_NAME || ' [' || OD.BASE_OBJECT_TYPE || ']' AS NVARCHAR(5000)) AS HIERARCHY_DISPLAY
            FROM 
                LINEAGE_HIERARCHY LH
                INNER JOIN SYS.OBJECT_DEPENDENCIES OD 
                    ON LH.BASE_SCHEMA_NAME = OD.DEPENDENT_SCHEMA_NAME
...
WITH RECURSIVE LINEAGE_HIERARCHY AS (
    ...
    SELECT 
        ...
        LH.LEVEL + 1 AS LEVEL,
        LH.LINEAGE_PATH || ' -> ' || OD.BASE_SCHEMA_NAME || '.' || OD.BASE_OBJECT_NAME AS LINEAGE_PATH,
        CAST(REPEAT('  ', LH.LEVEL + 1) || '└─ ' || OD.BASE_OBJECT_NAME AS NVARCHAR(1000)) AS HIERARCHY_DISPLAY
    FROM 
        LINEAGE_HIERARCHY LH
        INNER JOIN SYS.OBJECT_DEPENDENCIES OD 
            ON LH.BASE_SCHEMA_NAME = OD.DEPENDENT_SCHEMA_NAME
